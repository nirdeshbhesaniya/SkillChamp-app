...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    // Convert buffer to base64 string (Cloudinary accepts buffer streams)
    const fileBuffer = req.file.buffer.toString("base64");

    // Upload to Cloudinary from memory (no disk usage)
    const result = await uploadMediaToCloudinary(`data:image/png;base64,${fileBuffer}`);
